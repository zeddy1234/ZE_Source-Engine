...
AttemptRTV(client)
{
	...
	{
		CReplyToCommand(client, "%t", "RTV Not Allowed");
		return;
	}
		
	if (!CanMapChooserStartVote())
	{
		CReplyToCommand(client, "%t", "RTV Started");
		return;
	}
	
	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		CReplyToCommand(client, "%t", "Minimal Players Not Met");
		return;			
	}
	
	if (g_Voted[client])
	{
		CReplyToCommand(client, "%t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
	...
	CPrintToChatAll("%t", "RTV Requested", name, g_Votes, g_VotesNeeded);
...
StartRTV()
{
	...
	if (EndOfMapVoteEnabled() && HasEndOfMapVoteFinished())
	{
		...
		{
			CPrintToChatAll("%t", "Changing Maps", map);
