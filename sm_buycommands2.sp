...
int g_iNegevAmount[MAXPLAYERS + 1];
int g_iFlashAmount[MAXPLAYERS + 1];
int g_iDecoyAmount[MAXPLAYERS + 1];

int g_iRebuyWeapons[MAXPLAYERS + 1][21];

ConVar g_Cooltime;
ConVar g_RebuyPenalty;
...
public void OnPluginStart()
{
	...
	g_RebuyPenalty = CreateConVar("sm_rebuy_penalty", "500", "Rebuy Penalty");
	...
	g_NegevAmount = CreateConVar("sm_gc_negev_amount", "3", "max amount");
...
{
	for (int iCurrenClient = 1; iCurrenClient <= MaxClients; iCurrenClient++)
		for (int iCurrentWeapon = 0; iCurrentWeapon < 21; iCurrentWeapon ++)
			g_iRebuyWeapons[iCurrenClient][iCurrentWeapon] = 0;
	
	for (int iCurrenClient = 1; iCurrenClient <= MaxClients; iCurrenClient++)
		if (IsClientInGame(iCurrenClient))
			g_iSpam[iCurrenClient] = 0;
}

public Action Command_galil(int client, int args)
{
	if (!IsValidClient(client, false, false) || !ZR_IsClientHuman(client))return Plugin_Handled;
	...
	int gunprice = g_GalilPrice.IntValue + g_iRebuyWeapons[client][0] * g_RebuyPenalty.IntValue;
...
public Action Command_ak(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_AKPrice.IntValue + (g_iRebuyWeapons[client][1] * g_RebuyPenalty.IntValue);
...
public Action Command_sg556(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_SG556Price.IntValue + (g_iRebuyWeapons[client][2] * g_RebuyPenalty.IntValue);
...
public Action Command_nova(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_NovaPrice.IntValue + (g_iRebuyWeapons[client][3] * g_RebuyPenalty.IntValue);
...
public Action Command_xm1014(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_XM1014Price.IntValue + (g_iRebuyWeapons[client][4] * g_RebuyPenalty.IntValue);
...
public Action Command_mac10(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_Mac10Price.IntValue + (g_iRebuyWeapons[client][5] * g_RebuyPenalty.IntValue);
...
public Action Command_mp9(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_Mp9Price.IntValue + (g_iRebuyWeapons[client][6] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 MP9 Price:\x05 $%i", gunprice);
...
public Action Command_mp7(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_Mp7Price.IntValue + (g_iRebuyWeapons[client][7] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 Mp7 Price:\x05 $%i", gunprice);
...
public Action Command_mp5(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_MP5SDPrice.IntValue + (g_iRebuyWeapons[client][8] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 MP5SD Price:\x05 $%i", gunprice);
...
public Action Command_ump45(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_UMP45Price.IntValue + (g_iRebuyWeapons[client][9] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 UMP45 Price:\x05 $%i", gunprice);
...
public Action Command_m249(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_M249Price.IntValue + (g_iRebuyWeapons[client][10] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 M249 Price:\x05 $%i", gunprice);
...
public Action Command_negev(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_NegevPrice.IntValue + (g_iRebuyWeapons[client][11] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		if (g_iNegevAmount[client] < g_NegevAmount.IntValue)
		{
			SetClientMoney(client, cmoney - gunprice);
			
			PrintToChat(client, " \x04[BuyCommands]\x06 Negev Price:\x05 $%i", gunprice);
			g_iNegevAmount[client] += 1;
			g_iSpam[client] = GetTime() + g_Cooltime.IntValue;
			...
			g_iRebuyWeapons[client][11]++;
		}
		else PrintToChat(client, " \x04[BuyCommands]\x0F You can only purchase a maximum of %i per round.", g_NegevAmount.IntValue);
	}
	else if (cmoney < gunprice)PrintToChat(client, " \x04[BuyCommands]\x0F You don't have enough money.");
...
public Action Command_bizon(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_BizonPrice.IntValue + (g_iRebuyWeapons[client][12] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		
		PrintToChat(client, " \x04[BuyCommands]\x06 BIZON Price:\x05 $%i", gunprice);
...
public Action Command_p90(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_P90Price.IntValue + (g_iRebuyWeapons[client][13] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 P90 Price:\x05 $%i", gunprice);
...
public Action Command_scar(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_SCAR20Price.IntValue + (g_iRebuyWeapons[client][14] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 SCAR20 Price:\x05 $%i", gunprice);
...
public Action Command_awp(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_AWPPrice.IntValue + (g_iRebuyWeapons[client][15] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 AWP Price:\x05 $%i", gunprice);
...
public Action Command_ssg08(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_SSG08Price.IntValue + (g_iRebuyWeapons[client][16] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 SSG08 Price:\x05 $%i", gunprice);
...
public Action Command_m4a1(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_M4Price.IntValue + (g_iRebuyWeapons[client][17] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 M4 Price:\x05 $%i", gunprice);
...
public Action Command_m4a1s(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_M4SPrice.IntValue + (g_iRebuyWeapons[client][18] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 M4S Price:\x05 $%i", gunprice);
...
public Action Command_aug(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_AUGPrice.IntValue + (g_iRebuyWeapons[client][19] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 AUG Price:\x05 $%i", gunprice);
...
public Action Command_famas(int client, int args)
{
	...
	int cmoney = GetClientMoney(client);
	int gunprice = g_FAMASPrice.IntValue + (g_iRebuyWeapons[client][20] * g_RebuyPenalty.IntValue);
	
	if (cmoney > gunprice)
	{
		SetClientMoney(client, cmoney - gunprice);
		PrintToChat(client, " \x04[BuyCommands]\x06 FAMAS Price:\x05 $%i", gunprice);
