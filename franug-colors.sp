...
public OnPluginStart()
{
	...
	CreateConVar("sm_fcolors_version", DATA, "", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
{
	if(IsPlayerAlive(client) && ZR_IsClientHuman(client) && g_color[client] != 0) 
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
	if ( action == MenuAction_Select ) 
	{
		...
		if(IsPlayerAlive(client) && ZR_IsClientHuman(client)) SetEntityRenderColor(client, g_iTColors[g][0], g_iTColors[g][1], g_iTColors[g][2], g_iTColors[g][3]);
