...
int g_iStartTime = -1;
bool g_b10MinFired = false, g_b5MinFired = false, g_b1MinFired = false;

bool g_bRandomAdded = false;
...
public OnPluginStart()
{
	...
	g_Cvar_IncludeMaps = CreateConVar("mce_include", "6", "Specifies how many maps to include in the vote.", _, true, 2.0, true, 6.0);
	...
	RegConsoleCmd("sm_timeplayed", Command_TimePlayed, "Show time since map started");
...
public OnMapStart()
{
	...
	CreateTimer(10.0, Timer_TimeLeftCheck, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}

public Action Timer_TimeLeftCheck(Handle hTimer) {
	int timeleft;
	GetMapTimeLeft(timeleft);
	if(timeleft > 0) {
		int timeUntil10 = timeleft - (10*60);
		int timeUntil5 = timeleft - (5*60);
		int timeUntil1 = timeleft - (1*60);

		if(!g_b10MinFired && timeUntil10 > 0 && timeUntil10 < 30) {
			g_b10MinFired = true;
			CreateTimer(timeUntil10 + 0.5, Timer_10MinLeft, _, TIMER_FLAG_NO_MAPCHANGE);
		}
		if(!g_b5MinFired && timeUntil5 > 0 && timeUntil5 < 30) {
			g_b5MinFired = true;
			CreateTimer(timeUntil5 + 0.5, Timer_5MinLeft, _, TIMER_FLAG_NO_MAPCHANGE);
		}
		if(!g_b1MinFired && timeUntil1 > 0 && timeUntil1 < 30) {
			g_b1MinFired = true;
			CreateTimer(timeUntil1 + 0.5, Timer_1MinLeft, _, TIMER_FLAG_NO_MAPCHANGE);
		}
	}
}

public Action Timer_10MinLeft(Handle hTimer) {
	CPrintToChatAll("%t", "10 Minutes");
	g_b10MinFired = false;
}

public Action Timer_5MinLeft(Handle hTimer) {
	CPrintToChatAll("%t", "5 Minutes");
	g_b5MinFired = false;
}

public Action Timer_1MinLeft(Handle hTimer) {
	char szMap[PLATFORM_MAX_PATH];
	GetNextMap(szMap, sizeof(szMap));

	CPrintToChatAll("%t", "1 Minute", szMap);
	g_b1MinFired = false;
}

public Action Command_TimePlayed(int client, int argc) {
	int time = GetTime() - g_iStartTime;
	
	int hours = RoundToFloor(time / 3600.0);
	int mins = RoundToFloor((time % 3600) / 60.0);
	int secs = time % 60;
	...
	CPrintToChat(client, "%t", "Time Played", szTimeFormat);
}
...
public void OnClientSayCommand_Post(int client, const char[] command, const char[] sArgs)
{
	...
	if (strcmp(sArgs, "timeplayed", false) == 0)
	{
		Command_TimePlayed(client, 0);
	}
}
...
public OnMapEnd()
{
	...
	char last[PLATFORM_MAX_PATH];
	GetArrayString(g_OldMapList, GetArraySize(g_OldMapList) - 1, last, sizeof(last));

	if(!StrEqual(map, last))
...
public Action:Command_SetNextmap(client, args)
{
	...
	if (!IsMapValid(map))
	{
		CReplyToCommand(client, "%t", "Map was not found", map);
		return Plugin_Handled;
	}
...
public Event_RoundEnd(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(timeleft <= 0) {
		char szMap[PLATFORM_MAX_PATH];
		GetNextMap(szMap, sizeof(szMap));
		CPrintToChatAll("%t", "Map End", szMap);
	} 
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	if (IsVoteInProgress() || (g_NativeVotes && NativeVotes_IsVoteInProgress()))
	{
		...
		CPrintToChatAll("%t", "Cannot Start Vote", FAILURE_TIMER_LENGTH);
		new Handle:data;
	...
	else //We were given a list of maps to start the vote with
	{
		...
		for (new i=0; i<size; i++)
		{
			...
			else if (StrEqual(map, VOTE_RANDOM))
			{
				if (g_NativeVotes)
				{
					NativeVotes_AddItem(g_VoteMenu, VOTE_RANDOM, "Random Map");
				}
				else
				{
	...
	CPrintToChatAll("%t", "Nextmap Voting Started");
}
						   ...
						   const item_info[][2])
{
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		CPrintToChatAll("%t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else if (strcmp(map, VOTE_RANDOM, false) == 0)
	{
		char map2[PLATFORM_MAX_PATH];
		ArrayList tempMaps = new ArrayList(ByteCountToCells(PLATFORM_MAX_PATH));

		for(int i = 0; i < GetArraySize(g_MapList); i++) {
			GetArrayString(g_MapList, i, map2, sizeof(map2));
			tempMaps.PushString(map2);
		}

		GetCurrentMap(map2, sizeof(map2));
		RemoveStringFromArray(tempMaps, map2);

		for(int i = 0; i < GetArraySize(g_NextMapList); i++) {
			GetArrayString(g_NextMapList, i, map2, sizeof(map2));
			RemoveStringFromArray(tempMaps, map2);
		}

		for(int i = 0; i < GetArraySize(g_OldMapList); i++) {
			GetArrayString(g_OldMapList, i, map2, sizeof(map2));
			RemoveStringFromArray(tempMaps, map2);
		}

		tempMaps.GetString(GetRandomInt(0, tempMaps.Length - 1), map2, sizeof(map2));
		...
		if (g_ChangeTime == MapChange_MapEnd)
		{
			...
			if (g_NativeVotes)
			{
				NativeVotes_DisplayPass(menu, map2);
			}
		}
		else if (g_ChangeTime == MapChange_Instant)
		{
			...
			if (g_NativeVotes)
			{
				NativeVotes_DisplayPassEx(menu, NativeVotesPass_ChgLevel, map2);
			}
		}
		...
		CPrintToChatAll("%t", "Random Map", map2, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
		LogAction(-1, -1, "Voting for next map has finished. 'Random Map (%s)' was the winner", map2);
	}
	else if (strcmp(map, VOTE_DONTCHANGE, false) == 0)
	{
		...
		CPrintToChatAll("%t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		CPrintToChatAll("%t", "Nextmap Voting Finished", map, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
						   ...
						   const item_info[][2])
{
	...
	if (GetConVarBool(g_Cvar_RunOff) && num_items > 1 && g_RunoffCount < GetConVarInt(g_Cvar_MaxRunOffs))
	{
		...
		if (highest_votes == item_info[1][VOTEINFO_ITEM_VOTES])
		{
			...
			CPrintToChatAll("%t", "Tie Vote", GetArraySize(mapList));
		...
		else if (highest_votes < required_votes)
		{
			...
			CPrintToChatAll("%t", "Revote Is Needed", required_percent);
			SetupWarningTimer(WarningType_Revote, MapChange:g_ChangeTime, mapList);
			return;
		}
	}
...
public Handler_MapVoteMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		...
		case MenuAction_DisplayItem:
		{
			...
			if (StrEqual(map, VOTE_RANDOM, false))
			{
				Format(buffer, sizeof(buffer), "Random Map");
			}
...
stock AddMapItem(const String:map[])
{
	...
	if(!g_bRandomAdded && !g_bIsRunOff) {
		g_bRandomAdded = true;
		AddMapItem(VOTE_RANDOM);
	}
}
...
// new WriteToFile / Database stuff
public void LoadSavedFiles() {
	char szPath[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, szPath, sizeof(szPath), "configs/mapchooser_extended");

	if(!DirExists(szPath)) {
		CreateDirectory(szPath, 511);
	}
	
	BuildPath(Path_SM, szPath, sizeof(szPath), "configs/mapchooser_extended/last.txt");
	if(!FileExists(szPath)) {
		delete OpenFile(szPath, "w"); // create text file and delete handle
	}
	else {
		File lastFile = OpenFile(szPath, "r");

		char line[PLATFORM_MAX_PATH], prev[PLATFORM_MAX_PATH];
		while(lastFile.ReadLine(line, sizeof(line))) {
			if(!StrEqual(line, prev)) {
				TrimString(line);
				if(strlen(line) > 1) {
					PushArrayString(g_OldMapList, line);
				}
			}
			strcopy(prev, sizeof(prev), line);
		}
...
public void UpdateOldMapList() {
	char szPath[PLATFORM_MAX_PATH];
	BuildPath(Path_SM, szPath, sizeof(szPath), "configs/mapchooser_extended/last.txt");
	File lastFile = OpenFile(szPath, "w");

	char prev[PLATFORM_MAX_PATH];
	for(int i = 0; i < GetArraySize(g_OldMapList); i++) {
		char line[PLATFORM_MAX_PATH];
		GetArrayString(g_OldMapList, i, line, sizeof(line));

		if(!StrEqual(line, prev)) {
			TrimString(line);
			lastFile.WriteLine(line);
		}
		strcopy(prev, sizeof(prev), line);
	}
