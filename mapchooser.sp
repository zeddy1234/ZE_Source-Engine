...
{
	name = "MapChooser",
	author = "AlliedModders LLC & Strellic",
...
public Action Command_SetNextmap(int client, int args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "[SM] Usage: sm_setnextmap <map>");
	...
	ShowActivity2(client, "", "%t", "Changed Next Map", displayName);
...
void InitiateVote(MapChange when, ArrayList inputlist=null)
{
	...
	if (inputlist == null)
	{
		...
		for (int i=0; i<nominationsToAdd; i++)
		{
			...
			if(!firstAdd) {
				firstAdd = true;
				g_VoteMenu.AddItem(VOTE_RANDOM, "Random Map");
			}
		}
						   ...
						   const int[][] item_info)
{
	...
	else if (strcmp(map, VOTE_RANDOM, false) == 0)
	{
		...
		for(int i = 0; i < g_MapList.Length; i++) {
			g_MapList.GetString(i, map2, sizeof(map2));
		...
		for(int i = 0; i < g_NextMapList.Length; i++) {
			g_NextMapList.GetString(i, map2, sizeof(map2));
			RemoveStringFromArray(tempMaps, map2);
		}

		for(int i = 0; i < g_OldMapList.Length; i++) {
			g_OldMapList.GetString(i, map2, sizeof(map2));
			RemoveStringFromArray(tempMaps, map2);
		}
	...
	else
	{
		...
		CPrintToChatAll("%t", "Nextmap Voting Finished", displayName, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
						   ...
						   const int[][] item_info)
{
	if (g_Cvar_RunOff.BoolValue && num_items > 1)
	{
		...
		if (winningvotes < required)
		{
			...
			CPrintToChatAll("%t", "Starting Runoff", g_Cvar_RunOffPercent.FloatValue, info1, map1percent, info2, map2percent);
...
public void LoadSavedFiles() {
	...
	BuildPath(Path_SM, szPath, sizeof(szPath), "configs/mapchooser");

	if(!DirExists(szPath)) {
		CreateDirectory(szPath, 511);
	}
	
	BuildPath(Path_SM, szPath, sizeof(szPath), "configs/mapchooser/last.txt");
...
public void UpdateOldMapList() {
	...
	for(int i = 0; i < g_OldMapList.Length; i++) {
		char line[PLATFORM_MAX_PATH];
		g_OldMapList.GetString(i, line, sizeof(line));
