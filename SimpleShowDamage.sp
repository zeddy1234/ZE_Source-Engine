...
{
	g_cType = RegClientCookie("sm_showdamage_type", "Show Damage Display Type", CookieAccess_Private);
	...
	cvar = CreateConVar("sm_show_damage_fortnite_distance", "800.0", "Max distance before fortnite damage stops showing");
	cvar.AddChangeHook(CVarChanged_FortniteDist);
	g_fFortniteDist = cvar.FloatValue;

	cvar = CreateConVar("sm_show_damage_default", "0", "0 = fortnite, 1 = hinttext, 2 = hud");
	cvar.AddChangeHook(CVarChanged_DefaultType);
	g_iDefaultType = cvar.IntValue;
...
public void CVarChanged_DefaultType(ConVar cvar, const char[] oldValue, const char[] newValue)
{
	g_iDefaultType = cvar.IntValue;
	for (int client = 1; client <= MaxClients; client++) {
        if (IsClientInGame(client) && AreClientCookiesCached(client))
...
stock void ShowPRTDamage(int attacker, int victim, int damage, bool crit, bool late = false)
{
	...
	if(g_fFortniteDist != -1 && dist > g_fFortniteDist)
		return;
...
public Action Command_Hits(int client, int argc) {
	...
	menu.AddItem("0", g_iType[client] == 0 ? "HintText [ENABLED]" : "HintText");
	menu.AddItem("1", g_iType[client] == 1 ? "Fortnite [ENABLED]" : "Fortnite");
